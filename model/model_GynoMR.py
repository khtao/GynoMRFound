...
from data import AugmentTensor
...
class MAECLIP3D(nn.Module):
    ...
    def forward_classification(self, latent):
        if self.pool == 'mean':
            input_tokens = latent[:, 1:, :]
            input_tokens = input_tokens.mean(dim=1)
        else:
            input_tokens = latent[:, 0]
    ...
    def forward(self, imgs):
        ...
        if self.classification:
            x = self.forward_img_encoder_nomask(imgs)
            out = self.forward_classification(x)
            return loss, pred, mask, out
...
def mae_tiny(patch_size, img_size, masking_ratio, clip_dim, qkv_bias=True, norm_pix_loss=False,
             classification=False, n_outputs=None):
    model = MAECLIP3D(
        patch_size=patch_size,
        img_size=img_size,
        hidden_size=768,
